...
Subject to: $$D_t = Y_t - I_t - w_t L_t$$ $$K_{t+1} = (1 - \delta) K_t + I_t$$
...
With: $$D_t = F(K_t, L_t) - I_t$$ $$K_{t+1} = I_t + (1 - \delta) K_t$$
...
$$E[R_{t+1}^i] - R_{t+1}^f = \gamma \cdot \text{Cov}(R_{t+1}^i, R_{t+1}^k)$$

Where: - $R_{t+1}^k$ is the return on capital - $\gamma$ is the price of risk

### Derivation

From the SDF representation:

$$E[R_{t+1}^i] = R_{t+1}^f - \text{Cov}(R_{t+1}^i, M_{t+1})$$
...
$$K_{t+1} = I_t + (1 - \delta) K_t$$ $$Y_t = F(K_t, Z_t L_t)$$

Where $Z_t$ is investment-specific technology that affects productivity.

### Key Insight

Investment-specific shocks affect: 1. The productivity of new capital 2. The relative price of investment goods 3. The SDF through investment opportunities
...
Investment technology: $$K_{t+1} = \frac{I_t}{Q_t} + (1 - \delta) K_t$$
...
Subject to: $$D_t = P_t Y_t - Q_t I_t$$ $$K_{t+1} = \frac{I_t}{Q_t} + (1 - \delta) K_t$$
...
Utility maximization: $$\max E_0 \sum_{t=0}^\infty \beta^t u(C_t)$$

Subject to: $$P_t C_t + B_t = P_t w_t L_t + R_t B_{t-1} + \Pi_t$$

### Market Clearing

$$C_t + I_t = Y_t$$ $$B_t = 0$$ (no net supply of bonds)
...
Investment growth predicts: - Stock market returns - Bond yields - Risk premia
