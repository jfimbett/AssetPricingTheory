...
f\!\left(\sum_i \theta_i X_i\right) = \sum_i \theta_i f(X_i) = \sum_i \theta_i p_i.
$$
...
$$
\mathcal{C} = \left\{\, Y=\sum_i \theta_i X_i : \sum_i \theta_i p_i = 0 \,\right\} \subset \mathcal{X}.
$$
...
-   **Idea:** No-arbitrage means the set of zero-cost payoffs\
    $$\mathcal{C} = \{ Y : f(Y)=0 \}$$\
    cannot intersect the nonnegative cone of payoffs $$\mathcal{X}_+ = \{ X : X \ge 0 \}.$$ except at zero.
...
- We will use the separating hyperplane theorem in the dual space. 

---

## The dual space

-  The dual space $\mathcal{X}^*$ of a vector space $\mathcal{X}$ is the set of all linear functionals from $\mathcal{X}$ to its field of scalars (e.g., $\mathbb{R}$).
-  A linear functional is a function that satisfies linearity: for all $x, y \in \mathcal{X}$ and scalars $a, b$, we have $\varphi(ax + by) = a\varphi(x) + b\varphi(y)$.
-  The dual space is itself a vector space, with addition and scalar multiplication defined pointwise.
-  In finite-dimensional spaces, the dual space is isomorphic to the original space, meaning they have the same dimension and structure.

## Separating Hyperplane Theorem

- If two convex sets do not intersect, there exists a hyperplane that separates them.
- If they intersect only at a single point, there exists a hyperplane that touches them at that point.
- This hyperplane can be represented by a linear functional $\varphi$ such that:
    - $\varphi(x) \leq \alpha$ for all $x$ in the first set.
    - $\varphi(y) \geq \alpha$ for all $y$ in the second set.
    - For some scalar $\alpha$.
    - In two dimensions this is very simple to visualize, but it is true in any dimension.
- The theorem states that there exists a non-zero linear functional $\varphi \in \mathcal{X}^*$ such that 
$$
\varphi(Y) = 0 \quad \forall Y \in \mathcal{C}, 
\qquad
X \ge 0 \;\Rightarrow\; \varphi(X) \ge 0.
$$
- In other words, positive payoffs have positive prices.
...
-   Define inner product:\
    ...
    $$
-   An inner product is a way of multiplying (with some weights) two vectors to get a scalar. 
-   It is zero if the vectors are orthogonal (perpendicular).
-   Span of traded payoffs:\
    $$
    \mathcal{S} = \{ A\theta : \theta \in \mathbb{R}^N \}.
    $$

-   For a fixed $X$, any SDF $M$ decomposes uniquely as\
    $$
...
- In complete markets, no two different SDFs can have the same orthogonal projection to the space of traded payoffs, hence the SDF is unique.
- For the same $M$, $M_{\parallel}$ will be different for every asset that it prices, but there is only one $M$ that prices all at the same time.